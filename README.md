...
> yaml文件命名规则：序号+数据集代号+模型名+内插或外插.yaml，S指SST数据集，T指TurbulentFlow数据集，W指TurbineWake数据集

### 打包权重文件
运行`python pack_100.py --log-dir=<log文件相对路径>`

运行前确定 `.logs` 中的文件和权重文件目录 `.checkpoints_xx` 有一一对应