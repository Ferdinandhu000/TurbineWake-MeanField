...
from cfd.sensors import LHS, AroundCylinder, WakeShearLHS
...
class CFDDataset(Dataset, DatasetMixin):
    ...
    def __init__(
        ...
        noise_level: float,
        sensor_generator: Literal['LHS', 'AroundCylinder', 'WakeShearLHS'], 
        ...
        elif sensor_generator == 'WakeShearLHS':
            self.sensor_generator = WakeShearLHS(n_sensors=n_sensors)
            self.sensor_generator.seed = seed
            self.sensor_generator.resolution = resolution
        else:
        ...
        elif sensor_generator in ('LHS', 'WakeShearLHS'):
    ...
    def __write2disk(self) -> None:
        ...
        # Load data: shape (T, H, W, C) -> (T, C, H, W)
        data = torch.from_numpy(np.load(self.data_file)).cuda().float()
        # data is already (T, H, W, C) from preprocessing
        if data.ndim == 4:
        ...
        if sensor_frame_data.shape[-2:] != self.resolution:
        ...
        for sampling_id in range(self.n_samplings_per_chunk):
            ...
            for idx in tqdm(range(n_chunks), desc=f'Case {case_id + 1} | Sampling {sampling_id + 1}: '):
                ...
                if fullstate_sample.shape[-2:] != self.resolution:
