...
def plot_frame(
    ...
    title: str = '',
    filename: str = '',
    extent = [0, 1071, -252, 252],  # 物理空间范围 [xmin, xmax, ymin, ymax] — turbine wake downstream domain (m)
    dpi: int = 1200,
) -> None:
    ...
    if extent is not None:
        Lx_physical = extent[1] - extent[0]
        Ly_physical = extent[3] - extent[2]
        aspect_ratio: float = Ly_physical / Lx_physical
    else:
        aspect_ratio: float = frames_to_plot[0].shape[1] / frames_to_plot[0].shape[2]
    figwidth: float = 6.0
    figheight: float = 4.0 * num_plots
    fig, axs = plt.subplots(num_plots, 1, figsize=(figwidth, figheight), dpi=dpi)
    if num_plots == 1:
    ...
    # Ensure colorbar and labels are visible
