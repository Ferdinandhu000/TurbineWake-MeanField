...
def main():
    print("Loading Turbine Wake dataset (96 sensors)...")
    dataset = CFDDataset(
    ...
    print(f"\nRunning Table 4 multi-metric evaluation on {num_eval_chunks} chunks...")
    
    # Accumulate metrics
    # Shapes: (len(models), len(conditions))
    accum_maes = np.zeros((len(models), len(conditions)))
    accum_l2s = np.zeros((len(models), len(conditions)))
    accum_rmses = np.zeros((len(models), len(conditions)))
    ...
    for chunk_idx in range(num_eval_chunks):
        ...
        # Flatten from dim=2 (combines channels and spatial coordinates) to compute unified norm
        gt_norm = torch.linalg.norm(fv_gpu.flatten(2), dim=-1) # shape (1, 21)
        ...
        for cond_idx, (cond_name, noise, dropout) in enumerate(conditions):
            ...
            # Using exact same seed (1 + chunk_idx) for the embedding generator as CFDDataset would
            sensor_embedding = emb_gen(sensor_sample, seed=1 + chunk_idx).cuda()
            ...
            with torch.no_grad():
                for m_idx, name in enumerate(model_names):
                    ...
                    # MAE
                    mae = torch.mean(torch.abs(preds - fv_gpu)).item()
                    accum_maes[m_idx, cond_idx] += mae
                    
                    # Relative L2 Error (combined channels and spatial coordinates)
                    diff_norm = torch.linalg.norm((preds - fv_gpu).flatten(2), dim=-1) # shape (1, 21)
                    rel_l2 = (diff_norm / gt_norm).mean().item()
                    accum_l2s[m_idx, cond_idx] += rel_l2
                    
                    # RMSE
                    rmse = torch.sqrt(torch.mean((preds - fv_gpu) ** 2)).item()
                    accum_rmses[m_idx, cond_idx] += rmse
                    
        if chunk_idx % 100 == 99:
            print(f"  Processed {chunk_idx + 1}/{num_eval_chunks} chunks...")
            
    # Compute averages
    accum_maes /= num_eval_chunks
    accum_l2s /= num_eval_chunks
    accum_rmses /= num_eval_chunks
    
    # Print tables
    def print_metric_table(title, matrix):
        print(f"\n--- TABLE 4 (96 SENSORS) - {title} ---")
        print("| Model | Clean | Noise 5% | Noise 10% | Noise 20% | Dropout 5 | Dropout 10 | Dropout 20 |")
        print("| --- | --- | --- | --- | --- | --- | --- | --- |")
        for m_idx, name in enumerate(model_names):
            r = matrix[m_idx]
    ...
    print_metric_table("MAE", accum_maes)
    print_metric_table("Relative L2 Loss", accum_l2s)
    print_metric_table("RMSE", accum_rmses)
    
    # Save results as JSON
    results = {}
    for m_idx, name in enumerate(model_names):
        results[name] = {
            "MAE": accum_maes[m_idx].tolist(),
            "L2": accum_l2s[m_idx].tolist(),
            "RMSE": accum_rmses[m_idx].tolist()
        }
        
    out_json = PROJECT_DIR_W / "table_4_robustness_96_all_metrics_v2.json"
    with open(out_json, "w", encoding="utf-8") as f:
        json.dump(results, f, indent=4)
    print(f"\nSaved all metrics to {out_json}")
    
    # Save copy in artifacts dir
    artifacts_dir = Path(r"C:\Users\HJ000\.gemini\antigravity\brain\d559beff-90e2-4679-8d80-04bf042063a7")
    with open(artifacts_dir / "table_4_robustness_96_all_metrics_v2.json", "w", encoding="utf-8") as f:
        json.dump(results, f, indent=4)
