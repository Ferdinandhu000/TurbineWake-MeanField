...
class FNOBranchNet(nn.Module):
    ...
    def __init__(self, n_channels: int, n_fno_layers: int, n_hmodes: int, n_wmodes: int, embedding_dim: int, 
        ...
        # Determine patch size: find largest power-of-2 that divides both H and W
        def _best_patch(dim: int) -> int:
            for p in [8, 4, 2, 1]:
                if dim % p == 0:
                    return p
            return 1
        ph, pw = _best_patch(resolution[0]), _best_patch(resolution[1])
        patch_size = (ph, pw)
