...
def main():
    ...
    # Load data
    json_path = project_dir / "evaluation_results.json"
    with open(json_path, "r", encoding="utf-8") as f:
        data = json.load(f)
        
    sensor_counts = [64, 96, 128]
    ...
    out_file = output_dir / "turbine_sensor_mae_curves.png"
    plt.savefig(out_file, dpi=1200, bbox_inches='tight', transparent=True)
    plt.close()
    print(f"Saved Turbine sensor MAE bar chart to {out_file}")
    
    # Copy to artifacts for preview
    if artifacts_dir.exists():
        shutil.copy(out_file, artifacts_dir / "turbine_sensor_mae_curves.png")
