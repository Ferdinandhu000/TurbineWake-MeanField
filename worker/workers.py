...
class Worker:
    ...
    def _validate_embedding_generator(self, embedding_generator: Voronoi | SoftVoronoi | Mask | Vector) -> None:
        ...
        if isinstance(self.net, (FLRONetUNet, FLRONetFNO, FLRONetAFNO, FNO3D, FLRONetTransolver, FNO, AFNO, Transolver, UNet)):
...
class Trainer(Worker):
    ...
    def __init__(
        self, 
        net: FLRONetFNO | FLRONetAFNO | FLRONetUNet | FLRONetMLP | FNO3D | FLRONetTransolver | FNO | AFNO | Transolver | UNet,
        lr: float,
        ...
        self.net: FLRONetFNO | FLRONetAFNO | FLRONetUNet | FLRONetMLP | FNO3D | FLRONetTransolver | FNO | AFNO | Transolver | UNet = net
        self.lr: float = lr
        ...
        self.mask = None  # TurbulentFlowNet has no land mask; reserved for future use
    ...
    def train(
        ...
        if isinstance(self.net, (FLRONetFNO, FLRONetAFNO, FLRONetUNet, FLRONetMLP, FLRONetTransolver, FNO, AFNO, Transolver, UNet)):
        ...
        for epoch in range(1, n_epochs + 1):
            ...
            for batch, (
                ...
                if isinstance(self.net, (FLRONetFNO, FLRONetAFNO, FLRONetMLP, FLRONetUNet, FLRONetTransolver, FNO, AFNO, Transolver, UNet)):
                    # Forward propagation
...
class Predictor(Worker, DatasetMixin):
    ...
    def __init__(self, net: FLRONetFNO | FLRONetAFNO | FLRONetUNet | FLRONetMLP | FNO3D | FLRONetTransolver | FNO | AFNO | Transolver | UNet):
        self.net: FLRONetFNO | FLRONetAFNO | FLRONetUNet | FLRONetMLP | FNO3D | FLRONetTransolver | FNO | AFNO | Transolver | UNet = net.cuda()
    ...
    def predict_from_scratch(
        ...
        assert embedding_generator in ('Voronoi', 'SoftVoronoi', 'Mask', 'Vector')
        ...
        # load raw data
        data: torch.Tensor = self.load2tensor(case_dir) # already in GPU (B, C, H, W)
        n_channels = data.shape[1]
        
        # prepare sensor frames
        sensor_frames: torch.Tensor = data[sensor_timeframes]
        if sensor_frames.shape[-2:] != in_resolution:
            sensor_frames = F.interpolate(input=sensor_frames.flatten(0, 1), size=in_resolution, mode='bicubic')
            sensor_frames = sensor_frames.reshape(1, n_sensor_timeframes, n_channels, *in_resolution)
        
        # prepare sensor positions and dropout
        ...
        for t in available_fullstate_timeframes:
            if t is not None and (out_resolution is None or out_resolution == in_resolution): # not doing super-res and t has groundtruth
                fullstate_frame: torch.Tensor = data[t:t+1]
                if fullstate_frame.shape[-2:] != in_resolution:
        ...
        with torch.no_grad():
            ...
            if isinstance(self.net, (FLRONetFNO, FLRONetAFNO, FLRONetMLP, FLRONetUNet, FLRONetTransolver, FNO, AFNO, Transolver, UNet)):
                ...
                reconstruction_frames: torch.Tensor = self.net(
                    sensor_values=sensor_frames, 
                    out_resolution=(max_sensor_timeframe - min_sensor_timeframe, *(out_resolution or in_resolution)),
                )
        ...
        reduction_fn = (lambda x: compute_velocity_field(x, dim=0)) if n_channels >= 2 else (lambda x: x[0])
        ...
        for frame_idx in tqdm(range(reconstruction_frames.shape[0]), desc=f'{case_name}: '):
            ...
            if out_resolution is not None and out_resolution != in_resolution:  # super-resolution
                ...
                out_H, out_W = in_H[0] if isinstance(in_H, (list, tuple)) else in_resolution[0], in_resolution[1]
    ...
    def predict_from_dataset(self, dataset: CFDDataset, is_generate_plots: bool = False) -> None:
        ...
        with torch.no_grad():
            for sensor_timeframes, sensor_frames, fullstate_timeframes, fullstate_frames, case_names, sampling_ids in tqdm(dataloader):
                ...
                for frame_idx, timeframe in enumerate(fullstate_timeframes):
                    ...
                    frame_total_mae: torch.Tensor = self.mae(
                        input=reconstruction_frame.unsqueeze(0).unsqueeze(0), 
                        target=fullstate_frame.unsqueeze(0).unsqueeze(0),
                    )
        ...
        return sum(rmse_values) / len(rmse_values), sum(mae_values) / len(mae_values), sum(L2Loss_values) / len(L2Loss_values)
